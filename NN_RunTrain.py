...
for i in range(5):
    X[i*31:(i+1)*31] = X[i*31:(i+1)*31][:11]
...
X_train, Xt_train = normalize_bands(X_train, 
                          num_bands = N_BANDS, 
                          num_k_points = N_KPOINTS,
                          transformation = T_TYPE)
X_test, Xt_test  = normalize_bands(X_test, 
                          num_bands = N_BANDS, 
                          num_k_points = N_KPOINTS,
                          transformation = T_TYPE)
"""
...
#preds = antitransform_parameters(pd.DataFrame(np.array(preds)), t_test)
...
D_IN = 4
D_OUT = 155
D_HIDDEN_FC = [300, 1000, 1000, 1000, 1000]
P_DROPOUT = 0.25
...
optimizer_I = optim.Adadelta(our_Imodel.parameters(), lr=LEARNING_RATE)
optimizer_D = optim.Adadelta(our_Dmodel.parameters(), lr=LEARNING_RATE)
# Other optimizers
# - Adagrad
# - Adadelta
...
#%% Predictions
preds = predict(model_I, test_loader, device)
#preds = antitransform_parameters(pd.DataFrame(np.array(preds)), t_test)
#y_test = antitransform_parameters(y_test, t_test)

#%% Plots 
res = np.abs(y_test.to_numpy() - np.array(preds))/y_test.to_numpy()
...
#%%
param_preds = pd.DataFrame(np.array(predict(model_I, test_loader, device)))
fig, axs = plt.subplots(2, 2)
axs = np.ravel(axs)
for i in range(4):
    axs[i].hist(y_test[y_test.columns[i]])
    axs[i].hist(param_preds[i])
...
with torch.no_grad():
    for _, labels in test_loader:
        labels = labels.to(device)
        outputs = model_D(labels)
        predictions.extend(outputs.numpy())
#%%
predictions = pd.DataFrame(np.array(predictions))
predictions.columns = X.columns
predictions = unormalize_bands(predictions, Xt_test)
X_test = unormalize_bands(X_test, Xt_test)
plt.figure()
for i in range(5):
    plt.plot(predictions.iloc[15,i*31:(i+1)*31].values)
    plt.plot(X_test.iloc[15,i*31:(i+1)*31].values, '--')
